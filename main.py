...
for key, default in [
    (
        "pdf_name",
        None,
    ),  # filename — used to detect when the user switches PDFs
    ("tmp_pdf_path", None),  # path to the temp file PdfReader opens
    (
        "local_embedding",
        None,
    ),  # built index — avoids re-indexing on follow-up questions
...
with st.sidebar:
    ...
    st.markdown(
        "Ask questions about any PDF — fully local, no cloud required."
    )
    st.divider()
    ...
    st.markdown("#### Model")
    with st.status(
        "Loading LLM — this may take a minute on first run…", expanded=True
    ) as model_status:
        ai_model = load_ai_model()
        model_status.update(
            label="LLM ready", state="complete", expanded=False
        )
    ...
    if uploaded_file is not None:
        ...
        if uploaded_file.name != st.session_state.pdf_name:
            if st.session_state.tmp_pdf_path and os.path.exists(
                st.session_state.tmp_pdf_path
            ):
                os.unlink(
                    st.session_state.tmp_pdf_path
                )  # free the old temp file

            st.session_state.pdf_name = uploaded_file.name
            st.session_state.tmp_pdf_path = save_uploaded_pdf(uploaded_file)
            st.session_state.local_embedding = (
                None  # force re-indexing for new doc
            )
        ...
        if st.session_state.local_embedding is None:
            with st.status(
                "Processing document…", expanded=True
            ) as doc_status:
                ...
                st.write(
                    f"Building embedding index for {len(paragraphs)} paragraphs…"
                )
    ...
    st.divider()
    st.caption(
        "Powered by **Qwen2.5-3B-Instruct** + **MiniLM-L6-v2** · runs entirely on your machine"
    )
